...
export default class MainForm extends Component{
  ...
  onChangeDataAlignment=(event)=>{
    ...
    if(event.target.value === this.props.albumId){
      this.setState(()=>({
        alignColumn:{...this.state.alignColumn, alignId:event.target.checked ? this.props.rightAlign : this.props.leftAlign}
      }))
    }
    else if(event.target.value === this.props.id){
      this.setState(()=>({
        alignColumn:{...this.state.alignColumn, alignAlbunId:event.target.checked ? this.props.rightAlign : this.props.leftAlign}
      }))
    }
  }
  ...
  render(){
    ...
    const { allContent, items, hasMore, alignColumn, selectedWidth } = this.state
    ...
    return(
      <div style={tableStyle}>
        ...
        <DataTable rows={items} columns={this.getColumns()}
          onRowSelection={handleRowSelection} onRowClick={onRowClick} 
          dataAlign={alignColumn} onChangeAlignData={this.onChangeDataAlignment} 
          colWidth={selectedWidth} onChangeColWidth={this.onChangeColumnWidth} 
        />
